...
export function sumFertilizers(fertilizers: Fertilizer[], portions: number[]): Elements {
  const pairs = FERTILIZER_ELEMENT_NAMES.map((key) => [
    key,
    sum(portions.map((weight, index) => weight * fertilizers[index].elements[key])),
  ]);
  return Object.fromEntries(pairs);
}

export function getScoreElement(needElements: NeedElements, currentElements: Elements): Elements {
  const pairs = entries(needElements).map(([key, value]) => {
    const curVal = currentElements[key];
    let score = 100;
    if (curVal !== 0) {
      if (curVal < value) {
        score = value / curVal;
...
export function calculate_v4(
  ...
  const nFertilizers = Object.fromEntries(fertilizers.map((f) => [f.id, normalizeFertilizer(f)]));
  const macroFertilizers = fertilizers.filter(
    (f) => !MICRO_ELEMENT_NAMES.filter((n) => nFertilizers[f.id].elements[n]).length,
  );
  const microFertilizers = fertilizers.filter(
    (f) => MICRO_ELEMENT_NAMES.filter((n) => nFertilizers[f.id].elements[n]).length,
  );

  const microNeedElements = Object.fromEntries(
    entries(needElements).filter(
      ([el, v]) =>
  ...
  const microResult = calculate_v3(microNeedElements, microFertilizers, {
    ...options,
    ignore: { ...options.ignore, ...getFillElementsByType(true).macro },
  });
  ...
  const macroResult = calculate_v3(needElements, macroFertilizers, {
    ...options,
    ignore: { ...options.ignore, ...getFillElementsByType(true).micro },
  });

  const elementsEntries = entries(macroResult.elements).map(([e, v]) => [
    e,
    v + microResult.elements[e],
  ]);
  const elements = Object.fromEntries(elementsEntries);
  const deltaElements = FERTILIZER_ELEMENT_NAMES.map((e) => [
    e,
    (needElements[e] || 0) - elements[e],
  ]);
  ...
  const result: CalculateResult = {
    fertilizers: [...macroResult.fertilizers, ...microResult.fertilizers],
    elements,
...
export function calculate_v3(
  ...
  let count = 0;
  for (const f of combination(fertilizers)) {
    if (count > 3000) {
...
export function calculate_v2(
  ...
  const ignoredElements: Elements = getEmptyElements();
  const _priority = {
    ...getFillElementsByType(1000).macro,
    ...getFillElementsByType(3000).micro,
    ...ElementPriority,
  };
  for (const key of keys(ignoredElements)) {
    if (ignore[key]) {
  ...
  const weights: { [K: string]: FertilizerWeights } = Object.fromEntries(
    fertilizers.map((v) => [
      v.id,
      {
        id: v.id,
  ...
  for (const f of normalizedFertilizers) {
    const p = entries(f.elements)
      // Удаляем нулевой вес
      .filter((v) => v[1])
      .sort((a, b) => {
        const cmp1 = (_priority?.[b[0]] || 0) - (_priority?.[a[0]] || 0);
        const cmp2 = (xElements[a[0]] || 0) / a[1] - (xElements[b[0]] || 0) / b[1];
        return cmp1 + cmp2;
      });

    const primaryElement = p.filter(
      ([a]) => (xElements[a] || 0) > 0 && (needElements[a] || 0) > 0,
    )?.[0]?.[0];
    // Стараемся принять во внимание комплексные удобрения вроде акварина
    const skipFert =
      p.filter(([a]) => (needElements[a] || 0) <= 0 && !ignoredElements[a]).length === 1;
    if (!primaryElement || skipFert) {
      continue;
    }

    const m = Object.fromEntries(p);
    const weight = (xElements[primaryElement] || 0) / (m[primaryElement] * 10);
    const fInfo = fertilizerMap[f.id];
    ...
    if (fInfo.solution_concentration) {
      weights[f.id].volume = round(
        (weights[f.id].weight * 1000) / fInfo.solution_concentration,
        precision,
      );
    }
    const volume = weights[f.id].volume;
  ...
  const score_el: Elements = getScoreElement(needElements, calcElements);
  for (const [e, v] of entries(ignoredElements)) {
    if (v) {
      score_el[e] = 0;
    }
  }
  ...
  const ignored = sum(values(ignoredElements));
  const needElementsLength = keys(ignoredElements).length;
  const totalScore = round(
    100 / ((score_percent - (needElementsLength - ignored)) / (needElementsLength - ignored) + 1),
    2,
  );
...
export function calculate_v1(
  ...
  const time = new Date().getTime();
  let count = 0;
  const { accuracy = 0.1, max_iterations = 25, ignore = {} } = options || {};
  const precision = countDecimals(accuracy);
  ...
  const ignoredElements: Elements = getEmptyElements();
  for (const [key, flag] of entries(ignore)) {
    if (flag) {
      ignoredElements[key] = 1;
    }
  }
  ...
  for (const currentAccuracy of accuracyList) {
    ...
    for (const portions of it) {
      const n_el: Elements = sumFertilizers(fertilizers, portions);
      const score_el: Elements = getScoreElement(needElements, n_el);
      ...
      score = sum(Object.values(score_el).map((v) => v ** 2));
      if (best_score > score) {
    ...
    const newFertilizers: Fertilizer[] = [];
  ...
  for (const [k, v] of entries(calculatedElements)) {
