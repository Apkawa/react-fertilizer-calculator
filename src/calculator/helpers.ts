...
export function getFillElementsByType<T>(value: T): {
  macro: MacroElements<T>;
  micro: MicroElements<T>;
} {
  return {
    macro: {
      NO3: value,
...
export function calculatePPM(fertilizers: FertilizerWeights[], solution_volume = 1): number {
  return round(
    sum(
      fertilizers.map((f) => {
...
export function calculateToppingUp(options: CalculateToppingUpOptions): CalculateToppingUpResult {
  ...
  const ECn =
    -(cSol.EC * cSol.volume - nSol.EC * cSol.volume - nSol.EC * toppingVolume) / toppingVolume;
