 ...
 * @return new NPK
 */
export function convertProfileWithEC(
  npk: NPKElements,
  EC: number,
  ratio?: ElementsMatrixType,
): NPKElements {
  ...
  const r_N =
    (K_KCa * K_KMg) / (K_KCa * K_KMg * K_KN + K_KCa * K_KMg + K_KCa * K_KN + K_KMg * K_KN);
  const r_NH4 = (K_NH4NO3 * r_N) / (K_NH4NO3 + 1);
  const r_NO3 = r_N / (K_NH4NO3 + 1);

  const r_K =
    (K_KCa * K_KMg * K_KN) / (K_KCa * K_KMg * K_KN + K_KCa * K_KMg + (K_KCa * K_KN + K_KMg * K_KN));
  const r_Ca =
    (K_KMg * K_KN) / (K_KCa * K_KMg * K_KN + K_KCa * K_KMg + K_KCa * K_KN + K_KMg * K_KN);
  const r_Mg =
    (K_KCa * K_KN) / (K_KCa * K_KMg * K_KN + K_KCa * K_KMg + K_KCa * K_KN + K_KMg * K_KN);
  ...
  const r =
    ...
    (19.0 * M_Ca * M_K * M_Mg * m_NH4 +
      38.0 * M_Ca * M_K * M_NH4 * m_Mg +
      19.0 * M_Ca * M_Mg * M_NH4 * m_K +
      38.0 * M_K * M_Mg * M_NH4 * m_Ca);
  ...
  return Object.fromEntries(entries(newNpk).map(([el, v]) => [el, round(v, 1)]));
}
...
export function stringifyProfile(npk: NPKElements): string {
  const s = FERTILIZER_ELEMENT_NAMES.map((e) => {
    if (npk[e]) {
      let n = e as string;
      // Т.к. оперируем оксидами, то оксиды выводим как оксиды
      if (Object.hasOwn(NPKOxides, e)) {
