...
1. `pnpm test:e2e` (a11y-тест-кейсы): **`violations === []`** на всех основных маршрутах **и**
   в состояниях с открытыми модалками/sidebar (open-dropdown — отложен в `test.skip`:
   страница Example выпилена, «потом допилим»).
...
- **Open-dropdown a11y case** — решение (по ходу, user): страница Example (единственный «голый»
  Dropdown) выпилена; скан открытого дропдауна отложен в `test.skip`-заглушку, «потом допилим».
