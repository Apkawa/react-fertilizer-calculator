...
export function getProfileRatioMatrix(npk: NPKElements): ElementsMatrixType {
  const elMap: ElementInMatrix = {};
  for (const [El, ppm] of entries(npk)) {
  ...
  return Object.fromEntries(
    entries(elMap).map(([el, ppm]) => [
      el,
      Object.fromEntries(
        entries(elMap).map(([el2, ppm2]) => {
          let r = 1;
          if (el !== el2) {
            if (ppm2 === 0) {
              r = 0;
...
export function convertProfileWithRatio(
  npk: NPKElements,
  ratio: PartialElementsMatrix,
): NPKElements {
  const EC = calculateEC(npk);
  const v = getProfileRatioMatrix(npk);

  let newNPK: NPKElements = { ...npk };
  for (const [el1, toEls] of entries(ratio)) {
    for (const [el2, r] of entries(toEls)) {
      if (el1 === el2) {
        continue;
      }
      const f = (v: number, r: number) => v * r;
      v[el1][el2] = r;
      if (el1 === "N") {
        const _N = f(newNPK[el2 as MACRO_ELEMENT_NAMES] || 0, r);
 ...
 * ```
 */
export function getMultiElementRatio(
  npk: NPKElements,
  elements: ALLOWED_ELEMENT_IN_MATRIX[],
): MultiElementRatio {
  const ratio = getMultiRatio(
    elements.map((e) => {
      if (e === "N") {
