...
export function* calculateStartSaga(): Generator {
  ...
  const result = calculate_v4(formValues.recipe, formValues.fertilizers, {
    ignore: { ...ignore },
    accuracy,
    solution_volume,
    solution_concentration,
  ...
  if (topping_up_enabled && topping_up) {
    const toppingUpResult = yield select((state) => state.calculator.toppingUpResult);
    ...
    if (
      tRes.volume !== toppingUpResult.volume ||
      tRes.concentration !== toppingUpResult.concentration
    ) {
      doRecalculate = true;
    }
    if (doRecalculate) {
